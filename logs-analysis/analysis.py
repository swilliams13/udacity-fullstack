...
def logAnalysis():
    ...
    query2 = ("select name, sum(views) as totalViews from articles, authors,"
              " (select count(*) as views, path from log group by path) as "
              "log_views where articles.author = authors.id and "
              "log_views.path like concat('%', articles.slug) group by name"
    ...
    query3 = ("select round((num_errors/num_total)*100,1) as percentage, "
              ...
              "(status LIKE '4%' OR status LIKE '5%') group by date) "
              "as errors, "
              "(select cast(count(id) as decimal) as num_total, "
              "date_trunc('day',time) as date from log group by date) "
              "as total where total.date = errors.date and "
